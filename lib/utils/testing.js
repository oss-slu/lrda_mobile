...
import { StyleSheet, Text, TextInput, View, Animated, Image, TouchableOpacity, Dimensions, Platform, Keyboard } from "react-native";
...
const ExploreScreen = () => {
  ...
  const fetchMessages = useCallback(async () => {
    try {
      const fetchedNotes = searchResults
        ? searchResults
        : await ApiService.fetchMessages(globeIcon === "earth", globeIcon !== "earth", "someUserId");
      ...
      const fetchedMarkers = fetchedNotes.map((note) => {
        ...
        return {
          ...
          images: note.media.map((mediaItem) => ({ uri: mediaItem.uri })) || require("../../../assets/map_marker.png"), // Placeholder image replacement
  ...
  const [state, setState] = useState({
    ...
    categories: [
      {
        name: "Events",
        icon: <MaterialCommunityIcons style={styles.chipsIcon} name="calendar-multiple" size={18} />,
      },
      {
        name: "Cultural",
        icon: <Ionicons name="ios-globe-outline" style={styles.chipsIcon} size={18} />,
      },
      {
        name: "History",
      ...
      {
        name: "Modern",
        icon: <MaterialCommunityIcons name="city-variant-outline" style={styles.chipsIcon} size={18} />,
      },
      {
        name: "Religious",
        icon: <MaterialCommunityIcons name="church" style={styles.chipsIcon} size={15} />,
      },
    ],
    region: {
      latitude: 38.631393,
  ...
  useEffect(() => {
    ...
    (async () => {
      ...
      if (status !== "granted") {
        Alert.alert("Permission Denied", "Permission to access location was denied. This is required on this page", [
          {
            text: "Try Again",
            onPress: () => {
              (async () => {
          ...
          return (
            <MapView.Marker key={index} coordinate={marker.coordinate} onPress={(e) => onMarkerPress(e)}>
              <Animated.View style={[styles.markerWrap]}>
                <Animated.Image source={require("../../../assets/marker.png")} style={[styles.marker, scaleStyle]} resizeMode="cover" />
              </Animated.View>
            </MapView.Marker>
          );
        })}
        ...
        {showMapTypeOptions && (
          <>
            <TouchableOpacity onPress={() => setMapType("standard")}>
              <Text style={mapType === "standard" ? styles.selectedMapTypeText : styles.mapTypeText}>2D</Text>
            </TouchableOpacity>
            <TouchableOpacity onPress={() => setMapType("satellite")}>
              <Ionicons name="ios-globe" size={25} style={mapType === "satellite" ? styles.selectedMapTypeText : styles.mapTypeText} />
            </TouchableOpacity>
          </>
        )}
      </View>
