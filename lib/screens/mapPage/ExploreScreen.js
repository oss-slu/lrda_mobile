...
const ExploreScreen = () => {
      ...
      try {
        ...
        const regionTimeout = setTimeout(() => {
          if (mapIndex !== index) {
            ...
            console.log("Before animation: ", coordinate);
            ...
            _map.current.animateCamera(camera);
            */



            // Alright so from my testing the crashing is coming from the animateToRegion method above, some type of parsing problem is android
            console.log("Aniimation to region worked!")
          }
        }, 10);
      } catch (error) {
  ...
  const interpolations = state.markers.map((marker, index) => {
    try {
      ...
      // console.log("Ending Interpolation");
      return { scale };
    } catch (error) {
      console.error("An error occurred during interpolation:", error);
      return { scale };
    }
  });
