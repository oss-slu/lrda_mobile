...
const ExploreScreen = () => {
  ...
  const searchForMessages = async () => {
    ...
    // Process the searchResults as required
    // For instance, you can set them in the state to be displayed on the UI
    console.log(searchResults);
  };

  const fetchMessages = async () => {
    try {
      const isGlobeIconActive = globeIcon === "earth";
        const fetchedNotes = await ApiService.fetchMessages(
          isGlobeIconActive,
          !isGlobeIconActive,
          "someUserId"
        );
      ...
      const fetchedMarkers = fetchedNotes.map((note) => ({
        ...
        images: note.media.map((mediaItem) => ({ uri: mediaItem.uri })) || require("../../../assets/map_marker.png"), 
        time: formatToLocalDateString(new Date(note.time)) || "",
      }));
