...
const ExploreScreen = () => {
  ...
  const searchForMessages = useCallback(async () => {
    ...
    if (searchResults && searchResults.length > 0) {
      ...
      const latitude = parseFloat(firstResult.latitude) * 2 || 0;
      const longitude = parseFloat(firstResult.longitude) * 2 || 0;
...
const [state, setState] = useState({
        ...
        const regionTimeout = setTimeout(() => {
          if (mapIndex !== index) {
            ...
            if (state.markers[index]) {
              ...
              // From my guess, the error is coming here. Errors that I see about this on stack overflow come from the fact that android cannot parse strings like ios, causing breaks in android

              const { coordinate } = state.markers[index];
              _map.current.animateToRegion(
                {
                  ...coordinate,
                  latitudeDelta: state.region.latitudeDelta * 2,
                  longitudeDelta: state.region.longitudeDelta * 2,
                },
                350
              );
            }
  ...
  }, [mapAnimation, state.markers, state.region]);
