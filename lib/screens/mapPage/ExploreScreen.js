...
const ExploreScreen = () => {
  ...
  const fetchMessages = useCallback(async () => {
    try {
      ...
      // Sort notes by proximity to user's location
  ...
  // Function to sort notes by proximity
  ...
  // Listen to scroll position and animate map to corresponding marker
  useEffect(() => {
    mapAnimation.addListener(({ value }) => {
      const index = Math.floor(value / CARD_WIDTH + 0.3); // calculate index based on scroll position
          ...
          <>
            ...
            <TouchableOpacity onPress={() => setMapType("satellite")}>
              <Ionicons
                name="globe"
                size={25}
                style={mapType === "satellite" ? styles.selectedMapTypeText : styles.mapTypeText}
              />
            </TouchableOpacity>
          </>
        )}
        ...
        {state.markers.map((marker, index) => (
          <View style={styles.card} key={index}>
            ...
            <View style={styles.textContent}>
              <Text numberOfLines={1} style={styles.cardtitle} color={theme.text}>{marker.title}</Text>
