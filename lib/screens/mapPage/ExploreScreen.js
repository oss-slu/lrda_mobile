...
const ExploreScreen = () => {
  ...
  const fetchMessages = useCallback(async () => {
    try {
      const fetchedNotes = await ApiService.fetchMessages(globeIcon === "earth", globeIcon !== "earth", "someUserId");
      ...
      const fetchedMarkers = fetchedNotes.map((note) => {
        ...
        if (latitude === 0 || longitude === 0 || latitude < -90 || latitude > 90 || longitude < -180 || longitude > 180) {
          console.error(`Invalid coordinates for Note ${index}: Latitude: ${latitude}, Longitude: ${longitude}`);
      } else {
          console.log(`Valid coordinates for Note ${index}: Latitude: ${latitude}, Longitude: ${longitude}`);
      }
  ...
  const onMarkerPress = (mapEventData) => {
    ...
    console.log(`Marker Pressed: ID: ${markerID}, Latitude: ${marker.coordinate.latitude}, Longitude: ${marker.coordinate.longitude}`);
    ...
    <View style={styles.container}>
      ...
      >
        {state.markers.map((marker, index) => {
          console.log(`Rendering Marker ${index}: Latitude: ${marker.coordinate.latitude}, Longitude: ${marker.coordinate.longitude}`);
          const scaleStyle = {
            transform: [
              {
