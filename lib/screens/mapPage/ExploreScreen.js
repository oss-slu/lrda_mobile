...
const ExploreScreen = () => {
  ...
  useEffect(() => {
    const listenerId = mapAnimation.addListener(({ value }) => {
      const index = Math.floor(value / (CARD_WIDTH + 20) + 0.3);
      setCurrentIndex(index);
      if (!scrollEnabled.current) return;
      if (index >= state.markers.length) return;
      ...
      {shouldShowLoadMore && (
        <View style={styles.loadMoreContainer}>
          {isLoadingMore ? (
            ...
            <TouchableOpacity onPress={handleLoadMore} style={[styles.loadMoreButton, { backgroundColor: theme.homeColor }]}>
...
const styles = StyleSheet.create({
  ...
  searchBoxDark: { backgroundColor: "#333" },
  mapTypeSelector: {
    position: "absolute",
  ...
  loadMoreButton: { paddingVertical: 8, paddingHorizontal: 16, borderRadius: 8 },
  loadMoreButtonText: { fontSize: 16 },
});
